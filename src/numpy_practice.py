...
def replace_nonfinite_in_place(x):
    ...
    x[~np.isfinite(x)] = 0 #np.logical_not(x==np.inf)    
    #print('This is my answer: ',  x)
...
def replace_nans_out_of_place(x):
    ...
    #print('My answer is:    ', np.where(np.isnan(x),0,x))
...
def find_mode(x):
    ...
    #raise NotImplementedError
    # print('x =', x)
    # print('My answer is:   ', np.argmax(np.bincount(x.astype(int))))
